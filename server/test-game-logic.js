...
console.log("Board dimensions correct:", testBoard.length === 15 && testBoard[0].length === 15);
...
console.log("Deep copy successful:", JSON.stringify(copiedBoard) === JSON.stringify(testBoard));
...
console.log(
  testBoard
    .map((row) => row.map((tile) => tile.letter || tile.powerup || tile.crop || ".").join(" "))
    .join("\n")
);
